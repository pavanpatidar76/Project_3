...
/**
 * THe TimeTableModelHibImpl Class
...
public class TimeTableModelHibImpl implements TimeTableModelInt {
	...
	/*
	 * public TimeTableDTO checkByExamTime(Long courseId, Long subjectId, String
	 * semester, Date examDate, String examTime, String description) throws
	 * ApplicationException {
	 * 
	 * 
	 * Session session = null; TimeTableDTO dto = null; try { session =
	 ...
	 * criteria.add(Restrictions.like("examTime", examTime));
	 * criteria.add(Restrictions.like("description", description));
	 * 
	 * List list = criteria.list(); if (list.size() == 1) {
	 * 
	 * dto = (TimeTableDTO) list.get(0); }
	 * 
	 * } catch (Exception e) { log.error(e); throw new
	 * ApplicationException("Exception in duplicacy in timetable" + e.getMessage());
	 * } finally { session.close(); }
	 * 
	 * log.debug("Model Timetable End"); return dto; }
	 */	/*
	 * (non-Javadoc)
	 * 
	...
	public long add(TimeTableDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		long pk = 0;
		  ...
		  SubjectDTO subjectdto = subjectModel.findByPK(dto.getSubjectId()); CourseDTO
		  coursedto = courseModel.findByPK(dto.getCourseId());
...
//			System.out.println("5");
	...
	public TimeTableDTO checkByExamTime(long courseId, long subjectId, String semester, Date examDate, String examTime,
		...
		try {
			...
			log.error(e);
			throw new ApplicationException("Exception in duplicacy in timetable" + e.getMessage());
		} finally {
			session.close();
		}
